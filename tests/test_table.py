...
@pytest.mark.parametrize("choice, valid", [("md", True), ("html", True), ("yyz", False)])
...
def test_no_header_with_skip_rows(tmpdir):
    ...
    assert (
        ...
        if all(
            [
                len([x for x in errors if isinstance(x, ex.ColumnError)]) == 1,
                len([x for x in errors if isinstance(x, ex.RowError)]) == 2,
            ]
        )
        else False
    )
...
@pytest.mark.parametrize("skip, expected", [(1, True), (2, False)])
