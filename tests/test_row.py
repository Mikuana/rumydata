...
@pytest.mark.parametrize("value,err", [([1, 2, 3, 4, 5], rr.RowLengthLTE), ([1, 2, 3], rr.RowLengthGTE)])
...
def test_header_skip(basic):
    assert not table.Layout(basic, skip_header=True).check_header(["col1", "col2", "col4"])


@pytest.mark.parametrize("row_length,row,expected", [(1, ["1", "2"], False), (1, ["1"], True)])
...
@pytest.mark.parametrize(
    "row_length,row,expected", [(3, ["1", "2"], False), (2, ["1", "2"], True), (1, ["1", "2"], True)]
)
