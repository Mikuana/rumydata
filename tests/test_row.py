...
def test_row_bad(basic, value, err):
    assert table.Layout(basic)._has_error(value, err.class_exception(), rule_type=rr.Rule)
...
def test_header_bad(basic, value, err):
    assert table.Layout(basic)._has_error(value, err.class_exception(), rule_type=hr.Rule)
