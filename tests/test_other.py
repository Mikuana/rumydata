...
@pytest.fixture()
def readme_layout():
    return {"col1": field.Text(8), "col2": field.Choice(["x", "y", "z"], nullable=True), "col3": field.Integer(1)}


@pytest.fixture()
def readme_data(tmpdir):
    p = Path(tmpdir, "bobs_data.csv")
    p.write_text("\n".join(["col1,col2,col3", "abc,x,-1", "def,,0", "ghi,a,1"]))
    yield p.as_posix()
...
def test_file_not_exists(basic):
    assert CsvFile(rumydata.table.Layout(basic))._has_error("abc123.csv", ex.FileError)
...
@pytest.mark.parametrize(
    "row,kwargs", [(["1", "1"], {}), (["1", "1"], {"empty_row_ok": False}), (["", ""], {"empty_row_ok": True})]
)
...
@pytest.mark.parametrize("row", [(["1", ""]), (["", ""]), (["1", "1"])])
def test_ignore_row(row):
    """Test that ignore rows count as empty for the purpose of accepting empty rows"""
    lay = rumydata.table.Layout({"x": field.Ignore(), "y": field.Integer(1)}, empty_row_ok=True)
    assert not lay.check_row(row)
...
def test_empty_column_good(
    basic,
    basic_good,
    basic_good_with_empty,
    basic_good_with_trailing_empty_cols,
...
def test_complex_bad(bad_complex_file):
    ...
    expected_ex = ["NotNullIfCompare", "OtherCantExist", "OtherMustExist"]
    try:
...
@pytest.mark.parametrize("value,expected_output", [("1", "A"), ("1234", "AUL"), ("16384", "XFD")])
